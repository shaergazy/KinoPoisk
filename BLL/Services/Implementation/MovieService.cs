...
namespace BLL.Services.Implementation
{
    public class MovieService : TranslatableService<ListMovieDto, AddMovieDto, EditMovieDto, GetMovieDto, Movie, Guid, MovieDataTablesRequestDto>, IMovieService
    {
        ...
        public override IQueryable<Movie> FilterEntities(MovieDataTablesRequestDto request, IQueryable<Movie> entities = null)
        {
            ...
            {
                entities = entities.Where(s => s.Translations.Any(x => x.Value.ToUpper().Contains(searchTerm)));
                _logger.LogInformation("Filtered movies by search term: {SearchTerm}.", searchTerm);
            }

            if (!string.IsNullOrEmpty(request.Title))
            {
                entities = entities.Where(m => m.Translations.FirstOrDefault(x => x.FieldType == TranslatableFieldType.Title).Value.Contains(request.Title));
        ...
        {
            if (dto.Poster == null || dto.Translations == null)
            {
